...
class createChildThread (threading.Thread):
    def __init__(self, threadName, entityUtil, parent_entity_id, depth):
        threading.Thread.__init__(self)
        self.name = threadName+"D"+str(depth)
    ...
    def run(self):
        ...
        for entityindex in range(1, self.childcount+1):
            childentityname = "childentity-id-" + str(entityindex)
...
class EntityUtil():
    ...
    def createEntitiesWithoutParent(self):
        # Create new threads
        thread1 = createChildThread("Thread", self, self.parent_entity_id, self.depth)
...
et.createEntitiesWithoutParent()
et.attachParentEntityId()
